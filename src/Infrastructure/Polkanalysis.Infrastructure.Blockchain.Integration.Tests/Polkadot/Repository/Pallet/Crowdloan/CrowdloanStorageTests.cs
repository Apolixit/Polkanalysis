...
namespace Polkanalysis.Infrastructure.Blockchain.Integration.Tests.Polkadot.Repository.Pallet.Crowdloan
{
    public class CrowdloanStorageTests : PolkadotIntegrationTest
    {
        ...
        {
            var res = await _substrateService.Storage.Crowdloan.EndingsCountAsync(CancellationToken.None);
        ...
        {
            var res = await _substrateService.Storage.Crowdloan.FundsAsync(new Id(2094), CancellationToken.None);
            Assert.That(res, Is.Not.Null);
        }
        ...
        {
            var query = await _substrateService.Storage.Crowdloan.FundsQueryAsync(CancellationToken.None);
        ...
        {
            _ = await _substrateService.Storage.Crowdloan.NewRaiseAsync(CancellationToken.None);
        ...
        {
            var res = await _substrateService.At(20215034).Storage.Crowdloan.NewRaiseAsync(CancellationToken.None);
            Assert.That(res, Is.Not.Null);
