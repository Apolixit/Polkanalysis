...
namespace Substrate.NetApi.Model.Types.Base
{
        ...
        where T2 : IType, new()
    {
        ...
        public void CreateFromBitString(string bits)
        {
            ...
            if (IsLsb)
                s = s.Select(x => new string(x.Reverse().ToArray())).ToArray();

            if (s[0].Length != 8)
                s = s.Select(x => x.PadLeft(8, '0')).ToArray();
            ...
            {
                result.Add(Convert.ToByte(s[i], 2));
            }

            result.InsertRange(0, new CompactInteger((int)Math.Ceiling((double)bits.Length / 8)).Encode());
