...
namespace Polkanalysis.Infrastructure.Blockchain.Polkadot.Storage
{
    public class StakingStorage : PolkadotAbstractStorage, IStakingStorage
    {
        ...
        {
            return Map<ActiveEraInfoBase, ActiveEraInfo>(
        ...
        {
            var version = await GetVersionAsync(token);
            var input = _mapper.Map(version, key, _client.StakingStorage.ErasValidatorPrefsInputType(version));
            return Map<ValidatorPrefsBase, ValidatorPrefs>(
        ...
        {
            var accountId32 = await MapAccoundId32Async(account, token);
            return Map<NominationsBase, Nominations>(
        ...
        {
            var version = await GetVersionAsync(token);
            var input = _mapper.Map(version, key, _client.StakingStorage.NominatorSlashInEraInputType(version));
        ...
        public async Task<SpanRecord> SpanSlashAsync(BaseTuple<SubstrateAccount, U32> key, CancellationToken token)
        {
            ...
            var version = await GetVersionAsync(token);
            var input = _mapper.Map(version, key, _client.StakingStorage.SpanSlashInputType(version));
        ...
        {
            var version = await GetVersionAsync(token);
            var input = _mapper.Map(version, key, _client.StakingStorage.ValidatorSlashInEraInputType(version));
